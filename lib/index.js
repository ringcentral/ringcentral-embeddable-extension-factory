...
async function editFiles(from, res, template) {
  ...
  const pkgObjFirefox = copy(pkgObj)
  const contentScriptFirefox = copy(contentScript)
  delete pkgObjFirefox.background.persistent
  delete contentScriptFirefox.exclude_matches
  pkgObjFirefox.browser_action = pageAction
  pkgObjFirefox.permissions = filterPermissions(pkgObjFirefox.permissions)
  pkgObjFirefox.applications = {
    gecko: {
      id: generate() + '@dev-ringcentral.com',
      strict_min_version: '60.0'
    }
  }
  pkgObjFirefox.content_scripts = [contentScriptFirefox]
  ...
  writeFileSync(
    pkg.replace('manifest', 'manifest-firefox'),
    JSON.stringify(pkgObjFirefox, null, 2)
  )
